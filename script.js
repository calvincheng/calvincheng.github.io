...
function scrollToId(id) {
  ...
  if ("scrollBehavior" in document.documentElement.style) {
    ...
    let offset = parseInt(
      getComputedStyle(document.querySelector("html")).scrollPaddingTop
    );
...
document.addEventListener("scroll", (event) => {
  ...
  if (
    coverElement.getBoundingClientRect().bottom < 0 &&
  ...
  let numChapters = document.querySelector(
    ".chapter-list-wrapper ul"
  ).childElementCount;
  let n = numChapters;
  let height =
    document.querySelector(`h3:nth-of-type(${n})`).getBoundingClientRect().top +
    window.scrollY -
    offset;
  while (window.scrollY <= height - 1 && n > 1) {
    n--;
    height =
      document.querySelector(`h3:nth-of-type(${n})`).getBoundingClientRect()
        .top +
      window.scrollY -
      offset;
  }
  for (let i = 1; i <= numChapters; i++) {
    document
      .querySelector(`.chapter:nth-of-type(${i}) span`)
      .classList.remove("chapter-current");
  }
  document
    .querySelector(`.chapter:nth-of-type(${n}) span`)
    .classList.add("chapter-current");
});
